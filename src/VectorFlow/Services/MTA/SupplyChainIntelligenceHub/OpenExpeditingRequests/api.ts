...
export namespace OpenExpeditingRequestsService {
  ...
  export const addRemarkForExpedite = async (body:any) => {
    return await axios.post(process.env.REACT_APP_API_HOST + `api/mta/AddRemarkForExpedite`,body,{
      headers:{ 'Content-Type': 'application/json' }
    });
  }

  export const getRemarkDetailsForExpedite = async (body:any) => {
    return await axios.post(process.env.REACT_APP_API_HOST + `api/mta/GetRemarkDetailsForExpedite`,body,{
      headers:{ 'Content-Type': 'application/json' }
    });
  }
}
