...
describe('Dynamic Release Service', () => {
    ...
    it('should get orders for DRM when ao is not 0', async () => {
        mockedAxios.get.mockResolvedValueOnce({ data: 'test', status: 200 });
        const response = await DynamicReleaseManagementService.getDynamicReleaseData({ graph: 0, ao: 1, page: 1 });
        expect(response.status).toBe(200);
    });
    it('should get orders for DRM when wrong values', async () => {
        mockedAxios.get.mockResolvedValueOnce({ data: 'test', status: 200 });
        const response = await DynamicReleaseManagementService.getDynamicReleaseData({ graph: 3, ao: 3, page: 3 });
        expect(response.status).toBe(200);
    });

    it('should get orders for DRM when page is not 1', async () => {
        mockedAxios.get.mockResolvedValueOnce({ data: 'test', status: 200 });
        const response = await DynamicReleaseManagementService.getDynamicReleaseData({ graph: 0, ao: 0, page: 2 });
        expect(response.status).toBe(200);
    });
    ...
    it('should handle a failed API request gracefully', async () => {
        ...
        await expect(DynamicReleaseManagementService.getDynamicReleaseData({ graph: 1, ao: 1, page: 1 })).rejects.toThrow('Network Error');
    });
    ...
    it('should get orders for DRM when page is not 1, graph is 0, and ao is not 0', async () => {
        mockedAxios.get.mockResolvedValueOnce({ data: 'test', status: 200 });
        const response = await DynamicReleaseManagementService.getDynamicReleaseData({ graph: 0, ao: 1, page: 2 });
        expect(response.status).toBe(200);
