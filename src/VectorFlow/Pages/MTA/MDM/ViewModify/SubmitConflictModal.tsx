...
const SubmitConflictModal = (props: SubmitConflictModalProps) => {
  const { totalCount, recordCount, modificationCount, onFailure, onSuccess } =
    props;
  ...
  return (
    ...
    >
      <div
        className={SubmitDataTextContainer}
        style={{ paddingBottom: "30px" }}
      >
        {recordCount > 0 && (
          <>
            <b>{recordCount}</b> {recordCount > 1 ? "Records" : "Record"}{" "}
        ...
        {modificationCount > 0 && (
          <>
            <b>{modificationCount}</b> out of <b>{totalCount}</b> records are
            under modification already
          </>
        )}
      </div>
