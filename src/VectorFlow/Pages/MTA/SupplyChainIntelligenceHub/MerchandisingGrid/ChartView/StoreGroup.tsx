...
import { FloatingStoreDataType, TableLabelStatus } from "../../../.././../types/MCGrid";
import { getMCGridStoreIconColor, getMCGridStoreImgSrc } from "../../../../../../helpers/utils";
import { StoreHoveredIcon, StoreIcon } from "./Icon";
import { useUserData } from "../../../../../../context";

const StoreGroup = (props: { status:  TableLabelStatus; data: FloatingStoreDataType; setStatus: (status: TableLabelStatus) => void }) => {
  const { status, data, setStatus } = props;
  ...
  // Call setStatus when the group is clicked
  const handleGroupClick = () => {
    setStatus(status);  // Pass the status to the parent via setStatus
  };

  return (
    <ViewGridCell status={status} onClick={handleGroupClick}> {/* Added onClick */}
      <CellIconWrapper onMouseEnter={() => setIsHovered1(true)} onMouseLeave={() => setIsHovered1(false)}>
        {isHovered1 ? (
          <React.Fragment>
      ...
      <FloatingIconWrapper style={{ border: `solid 1px ${getMCGridStoreIconColor(status)}`, cursor: "pointer" }}>
        <CellIconWrapper onMouseEnter={onDetailMouseIn} onMouseLeave={onDetailMouseOut}>
          {isHovered2 ? (
            <React.Fragment>
              <StoreHoveredIcon color={getMCGridStoreIconColor(status)} value={data.available["grid-points"]} />
              <CellIconLabel>Store Grid Points</CellIconLabel>
            </React.Fragment>
          ) : (
            <React.Fragment>
              <StoreIcon value={data.available.stores} color={getMCGridStoreIconColor(status)} />
              <CellIconLabel>Stores</CellIconLabel>
            </React.Fragment>
          )}
        </CellIconWrapper>
      ...
      {isOpen && (
        <Portal wrapperId="mc-grid">
          <ToolTipWrapper onMouseLeave={onMouseOut} style={{ ...toolTipPosition, transform: "translateX(-40%)" }}>
            <InventoryToolTipContent>{inventoryStatus}</InventoryToolTipContent>
          </ToolTipWrapper>
        </Portal>
      )}
