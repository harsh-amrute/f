...
import { convertUiConfigToOptions, mapRRRColorBandWiseFieldsToColDefs } from "../../../../../helpers/utils"
import { notifyError} from "../../../../../helpers/notify"
// import { toast } from "react-toastify";

import useBPRFilter from "../../../../../hooks/useBPRFilter";
import { defaultAgGridSideBarForBPR } from "../../../../../helpers/BPRConstants";
import { useGetDailyData, useGetState, useGetUiConfig } from "../../../../Services/MTA/SupplyChainIntelligenceHub/BPR"
...
const useRRRColorBandwise =()=>{
    ...
    const [RRRBandwiseRowData,setRRRBandwiseRowData] = useState<any[]>(RowData)

    const [RRRColorBandWiseColumns,setRRRColorBandWiseColumns] = useState<Array<ColDef>>([])

    const [RRRBandwiseDataCount, setRRRBandwiseDataCount]=useState<any>(30);
    ...
    const getBandwiseDataCount=async (filter?:any) => {
        ...
        setRRRBandwiseDataCount(rowDataCount?.data?.recordCount)
    }

    const getRRRColorBandWiseUiConfig = async()=>{
        try{
            const response = await getUiConfig('RRR_OA')
            setRRRColorBandWiseColumns(mapRRRColorBandWiseFieldsToColDefs(response.data.data,onOpenDailyDataGraph))
        }catch(err:any){
    ...
    const tempAgGridProps:AgGridReactProps = {
        onRowDataUpdated:(event)=>{
         if(tempDownloadData) event.api.exportDataAsExcel({fileName:'RationedRequirementReportColorBandwise', columnKeys:ref.current?.api.getAllDisplayedColumns().map((c)=>c.getColId())});
        }
      };
