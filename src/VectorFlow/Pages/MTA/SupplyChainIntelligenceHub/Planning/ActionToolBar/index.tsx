...
interface ActionToolBarProps {
    ...
    hideGoBack?: boolean,
    visibilityToPP?: boolean,

}

const ActionToolBar = ({ view, currentTab, tabsList, onFloatingTabChange, onGoBack, onViewChange, currCategory, disableChartAndGridViewToggle, onApplyFilter, hideGoBack, visibilityToPP }: ActionToolBarProps) => {
    ...
    const renderFilter = () => {
        switch (currCategory) {
            ...
            case 'DBMNorm':
                ...
                break;
            case 'PP':
                if (pathname === '/procrmenPlanning/planning') {
                    return <VFMultiFilter onApplyFilter={handleApplyFilter} multiFilter={multiFilter} onGoBack={() => toggleFilter(false)} setMultiFilter={setMultiFilter} productFilterActive={true} supplyChainNodeFilterActive={true} locationFilterActive={true} supplyChainForLocationCheckBoxList={MultiFilterSupplyChainCheckboxList} supplyChainForChildrenOfCheckBoxList={MultiFilterSupplyChainCheckboxList.filter((m) => ['1', '3', '4'].includes(m.id))} />;
                }
                break;
            default:
                <></>
        ...
        <>
            ...
            }
            {
                ...
                <SCTaskBarContainer>
                    <SCTaskFilterContainer>
                        {
                            hideGoBack != true ? <SCGoBackContainer onClick={onGoBack}>
                                <img src="/assets/img/VectorFLOW/BPR/goback.svg" alt="" onClick={onGoBack} />
                                <SCGoBackText><b>Go Back</b></SCGoBackText>
                            </SCGoBackContainer> : null
                        }

                        {currCategory === 'GuidedInsight' ? null :
                            visibilityToPP === true ? <>
                                <ReleaseText>Release date till</ReleaseText>
