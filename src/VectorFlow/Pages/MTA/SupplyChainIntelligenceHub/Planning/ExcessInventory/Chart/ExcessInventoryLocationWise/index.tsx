...
const ExcessInventoryLocationWise = ({ data }: ExcessInventoryProps) => {
  ...
  const chartParams1 = useMemo(
    () => createChartParams("sku", currency),
    [currency]
  );
  ...
  const colDefs1 = mapUIConfigToColdefs1(
    data["topTenLocationsWithExcessInventorySkuCount"]["uiconfig"]
  );
  ...
  const colDefs2 = mapUIConfigToColdefs2(
    data["topTenLocationsWithExcessInventoryValue"]["uiconfig"]
  );
  ...
  useEffect(() => {
    const formattedRowData1 = sortData(
      convertToInt(data["topTenLocationsWithExcessInventorySkuCount"]["data"], [
        "SKUCounts",
      ]),
      "SKUCounts"
    );
    ...
    const formattedRowData2 = scaleDown(
      sortData(
        convertToInt(data["topTenLocationsWithExcessInventoryValue"]["data"], [
          "SumOfAmount",
        ]),
        "SumOfAmount"
      ),
    ...
    setChartThemeOverridesG2(
      generateChartOptions(formattedRowData2, chartParams2, undefined)
    );
  }, []);
