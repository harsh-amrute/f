...
import { calenderHeading } from "../../Poogi/InsightAndTrends/ResourceUtilization/styles.css";
import { CalenderWrapper } from "../../../MTA/InsightsAndTrends/ResearchInsights/styles.css";
import CustomCalenderDay from "../../Poogi/InsightAndTrends/ResourceUtilization/CustomCalenderDay";
import { DayPicker } from "react-day-picker";

function CalenderModalCard({
  selectedData,
  setSelectedData,
  isModalOpen,
  setIsModalOpen,
    ...
    if (selectedData.rb === "Monthly") {
      selectedData.dow.forEach((day: any) => {
        ...
        while (currentMonth.isSameOrBefore(endDate, "month")) {
          ...
          if (
            matchingDate &&
            matchingDate.isBetween(startDate, endDate, "day", "[]")
          ) {
  ...
  const safeMaxDate = useMemo(() => {
    const datesToCompare: any = [
      ...
      selectedData?.sd
        ? moment(selectedData.sd, "YYYY-MM-DD").add(1, "day")
        : null,
                ...
                <DayPicker
                  ...
                  components={{
                    ...
                    Day: (props: any) => {
                      ...
                      const opacity: string = disableDates?.includes(
                        formattedDate
                      )
                        ? "0.6"
                        : "1";
