...
const ConflictErrorCellRenderer = (params: any) => {
  ...
  const conflictTextColor =
    themeUi === "REGALBLAZE" ? "rgb(164 104 6)" : "rgb(130, 15, 76)";
  ...
  const [isToolTipOverflowing, setIsToolTipOverflowing] =
  ...
  const onMouseIn = (e: any) => {
    ...
    setTimeout(() => {
      if (toolTipRef.current) {
        ...
        const tooltipLeft =
          left * currGridZoom * currScreenZoom +
          (targetWidth / 2) * currGridZoom * currScreenZoom;
    ...
    >
      ...
      {params.data.users && isToolTipOpen && (
        <Portal wrapperId="conflict-tooltip">
          <div
            className={ConflictErrorToolTipWrapper}
            ref={toolTipRef}
          ...
          >
            ...
            {params.data.users.map((user: any, index: number) => (
              <div
                key={index}
                className={ConflictErrorToolTipSection}
                style={{
                  borderBottom:
                    index < params.data.users.length - 1
                      ? "1px solid gray"
                      : "unset",
                }}
              >
                <p className={ConflictErrorText}>
                  <b>User</b> : {user.user}
                </p>
                <p className={ConflictErrorText}>
                  <b>{params.colDef?.headerName}</b> :{" "}
                  {!user.data[currColumn] ? "NULL" : user.data[currColumn]}
                </p>
              </div>
            ))}
