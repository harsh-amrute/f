...
const ExpeditingMTO = (props: {
  ...
  date: string;
  rmHorizon: any;
  setRmHorizon: (day: any) => void;
  getFilterData: () => void;
  appliedFilters: any;
}) => {
  const { date, rmHorizon, setRmHorizon, getFilterData, appliedFilters } =
    props;
  ...
  const getRMHorizonBasedData = async () => {
    ...
    try {
      ...
      notifyError(
        "Failed to fetch RM expediting data. Please try again later."
      );
    }
  };
  ...
  const graphTitleJSX = (
    ...
    >
      <span style={{ fontWeight: 500 }}>
        Top 10 Raw Materials Impacting Orders With Release Date In Selected
        Horizon
      </span>
  ...
  return (
    ...
    >
      <SplitGraphContainer
        ...
        tableTitle={`Top 10 Raw Materials Impacting Orders With Release Date In Selected Horizon (${moment(
          date
        ).format("Do MMMM YYYY")} - ${moment(date)
          .add(rmHorizon, "days")
          .format("Do MMMM YYYY")})`}
        options={options}
        colDef={colDef}
