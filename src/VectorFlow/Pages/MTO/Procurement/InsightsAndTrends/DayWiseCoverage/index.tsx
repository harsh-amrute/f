...
const DayWiseCoverage = () => {
  ...
  const { mutateAsync: getUserUIReportConfigData, isLoading: isGetUserConfig } =
    useGetUserUIConfigData();
  const { mutateAsync: getData, isLoading: isCalenderLoading } =
  ...
  const getColor = (id: string) => {
    ...
    return calenderData?.[id]?.oc === calenderData?.[id]?.fk
      ? Colors.Green
      : Colors.Red;
  };
      ...
      >
        <thead>
          <tr>
            <td
              colSpan={2}
              style={{ borderBottom: "1px dashed white", paddingRight: "4rem" }}
            >
              Details
            </td>
          </tr>
        ...
        <tbody>
          <tr>
            <td>No of Orders</td>
            <td>{calenderData?.[id]?.oc}</td>
          </tr>
          <tr>
            <td>Full kit</td>
            <td>{calenderData?.[id]?.fk}</td>
          </tr>
          <tr>
            <td>Partial kit</td>
            <td>{calenderData?.[id]?.pk}</td>
          </tr>
          <tr>
            <td>No kit</td>
            <td>{calenderData?.[id]?.nk}</td>
          </tr>
        </tbody>
      </table>
    );
  ...
  const formatNumber = (number: number) =>
    number % 1 !== 0 ? Math.round(number * 100) / 100 : number;
    ...
    if (calenderData) {
      ...
      const data = Object.entries(analytics).map((entry: any) => {
        ...
        return [
          ...
          greens,
          reds,
          formatNumber((greens / (greens + reds)) * 100 || 0),
        ];
      });
  ...
  const {
    ...
    isMatReqLoading,
  } = useMaterialReq(null, selectedDate);
  ...
  const ExcelExport = () => {
    currentGridRef?.current?.api?.exportDataAsExcel({
      fileName: `Day_Wise_Coverage${format(Date.now(), "dd/MM/yyyy")}`,
    });
  };
        ...
        {calenderData?.[selectedDate] ? (
          ...
          <div className={AnimationWrapper}>
            ...
            <div className={HelperText}>
              Please select the <strong>Dates</strong> from above to view{" "}
      ...
      {calenderData?.[selectedDate] && (
        ...
        >
          ...
          <svg
            ...
            width="15.917"
            height="15.917"
            viewBox="0 0 15.917 15.917"
          >
            <path
              id="Path_10654"
              data-name="Path 10654"
              d="M16.194,8.959a.724.724,0,0,0-.724.724v4.341a1.447,1.447,0,0,1-1.447,1.447H3.894a1.447,1.447,0,0,1-1.447-1.447V3.894A1.447,1.447,0,0,1,3.894,2.447H8.235A.724.724,0,0,0,8.235,1H3.894A2.894,2.894,0,0,0,1,3.894V14.023a2.894,2.894,0,0,0,2.894,2.894H14.023a2.894,2.894,0,0,0,2.894-2.894V9.682A.723.723,0,0,0,16.194,8.959Z"
            ...
            <path
              ...
              d="M19.9,1.444a.716.716,0,0,0-.135-.2c-.009-.009-.012-.022-.021-.031s-.022-.012-.031-.021A.679.679,0,0,0,19.235,1H14.894a.724.724,0,0,0,0,1.447h2.594L12.212,7.723a.723.723,0,1,0,1.023,1.023L18.512,3.47V6.064a.724.724,0,1,0,1.447,0V1.723a.716.716,0,0,0-.056-.279Z"
              transform="translate(-4.041 -1)"
              fill="#b93b7e"
            />
          </svg>
        </div>
