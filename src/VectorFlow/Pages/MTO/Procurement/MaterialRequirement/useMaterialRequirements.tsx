...
import {
  useGetMaterialRequirementDetails,
  useGetMaterialRequirementDetailsDatewise,
  useGetMaterialRequirementDetailsForExcelExport,
} from "../../../../../VectorFlow/Services/MTO/Procurement/MaterialRequirement";
...
const useMaterialReq = (appliedFilters: any, forDate?: string) => {
  ...
  const { mutateAsync: getMaterialRequirementDataExcelExport } =
    useGetMaterialRequirementDetailsForExcelExport();
    ...
    net_r: {
      valueFormatter: (params: any) =>
        !_.isEmpty(params.data)
          ? Math.max(0, Number(params.data.net_r))
          : undefined,
    },
  };
  ...
  const {
    mutateAsync: getMaterialRequirementData,
    isLoading: isMatReqLoading,
  } = useGetMaterialRequirementDetails();
  const {
    mutateAsync: getMaterialRequirementDataDayWise,
    isLoading: isMatReqDayWiseLoading,
  } = useGetMaterialRequirementDetailsDatewise();
          ...
          <>
            ...
            <div className={TableWrapper}>
              <VFTable
                ...
                maintainColumnOrder
                onFilterChanged={() => {
                  Object.keys(gridRef?.current?.api?.getFilterModel())?.length >
                  0
                    ? setIsDisabled(false)
                    : setIsDisabled(true);
                }}
