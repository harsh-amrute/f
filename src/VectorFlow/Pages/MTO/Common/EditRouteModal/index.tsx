...
import {
  useFetchFOLGap,
  useSaveRouteData,
} from "../../../../Services/MTO/Production/DynamicReleaseManagement";
...
const EditRouteModal = ({
  ...
  const { mutateAsync: getRouteDetails, isLoading: isGetRouteDetails } =
    useGetRouteDetails();
  const { mutateAsync: getCCRGroupMaster, isLoading: isGetCCRGroupMaster } =
    useGetCCRGroupMaster();
  const {
    mutateAsync: getCCRItemTypeMappingMaster,
    isLoading: isGetgetCCRItemTypeMappingMaster,
  } = useGetCCRItemTypeMappingMaster();
  const { mutateAsync: getFOLData, isLoading: isGetFOLData } = useGetFOLData();
  const { mutateAsync: getLineCCRDetails, isLoading: isGetLineCCRDetails } =
    useGetLineCCRDetails();
  const {
    mutateAsync: saveRouteData,
    isLoading,
    isSuccess,
    isError,
  } = useSaveRouteData();
  const { mutateAsync: fetchFolGapData, isLoading: isFetchFolGapData } =
    useFetchFOLGap();
  ...
  const getMastersData = async () => {
    try {
      ...
      const CCRItemTypeMappingMasterData = Object.values(
        CCRItemTypeMappingMaster?.data?.data
      );
      ...
      setMasters({
        ccrGroups,
        CCRItemTypeMappingMaster: CCRItemTypeMappingMasterData,
      });
    } catch (error) {
      console.log(error);
  ...
  function convertToRequiredFormat(routes: Route[], lineCcr: LineCcr): any {
    ...
    routes.forEach((e: any, i) => {
      ...
      const lineCCRPendingQty =
        lineCcr[orderDetails?.orderKey]?.[CCRId]?.pcqty || 0;
      const CCRPendingQty = lineCCRPendingQty || orderPendingCCRQty;
      const ccrItem = masters?.CCRItemTypeMappingMaster.find(
        (ccr: any) => ccr.ccrId === CCRId && ccr.it == orderDetails.itemTypeId
      );
      const orderLoad = Math.ceil(
        (ccrItem.tt || 0) *
          (CCRPendingQty && CCRPendingQty >= 0
            ? CCRPendingQty
            : orderPendingCCRQty)
      );
  ...
  const calculateCCGroups = () => {
    ...
    const filteredCCRGroups = masters?.ccrGroups
      ?.map((ccrGroup: any) => {
        // Filter CCRs within each group
        const filteredCCRs = ccrGroup.ccrs.filter((ccr: any) => {
          return (
            ccr.plant_id === orderDetails.plantId && validCCRs.has(ccr.value)
          );
        });
  ...
  const isSaveDisabled = useMemo(() => {
    ...
    return (
      !route.length ||
      route.some((routeItem: any) => !routeItem[0] || !routeItem[1])
    );
  ...
  return (
    ...
    >
      ...
      {(isFetchFolGapData ||
        ...
        isGetLineCCRDetails ||
        isGetCCRGroupMaster ||
        isGetgetCCRItemTypeMappingMaster) && (
        <OverlayLoader message="Loading, please wait ..." />
      )}
      {showFOLGapDetails ? (
        <div className={folGapContentWrapper}>
          <span className={folGapDetailHeader}>
            Are you sure you want to change the routes?
          </span>
          <span className={folGapDetailHeaderInfo}>
            *This change will be permanent but will not have any effect on due
            dates
          </span>
          <div className={folGapDetailDiv}>
            <span className={folGapDetailHeaderInfoMain}>
        ...
        <div className={routeContentWrapper}>
          <div className={text}>You can change route by selecting CCR from drop-down</div>
          ...
          <div className="chart-wrapper">
            ...
            <div
              ...
              style={{
                overflowX:
                  chartoptions?.data?.length > 10 ? "scroll" : "hidden",
              }}
            >
              <AgCharts
