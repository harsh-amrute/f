import { VFWrapper } from "./styles.css";
import {
  SaveBtnWrapper,
  SaveBtn,
  saveBtnAccentVar,
} from "../../Poogi/ReasonOrderChange/style.css";
...
const GridView = memo(
  ({
    ...
    const rowsPerPage =
      userPageSize ||
      Number(process.env.REACT_APP_MTO_BM_REPORT_ROWS_PER_PAGE) ||
      pagination.mtoPageSize;
    ...
    const accent = globalStyles.chooseThemeColor[themeUi]?.color4 ?? "#820f4c";

    return (
      <>
        <div className={`${VFWrapper} wrapper-overall`}>
          <VFTable
            {...agGridProps}
            maintainColumnOrder
            pagination={false}
            ...
            detailCellRendererParams={
              detailCellRendererParamsConfig?.detailCellRendererParams
            }
            ...
            ref={reference}
            onFilterChanged={() => {
              Object.keys(reference?.current?.api?.getFilterModel())?.length > 0
                ? setIsDisabled(false)
                : setIsDisabled(true);
            }}
          />
        ...
        {saveBtn && canAddComments && (
          <div
            className={SaveBtnWrapper}
            style={{ margin: "1rem 0", padding: 0, cursor: "pointer" }}
          >
            <div
              className={SaveBtn}
              onClick={updateReason}
              style={assignInlineVars({ [saveBtnAccentVar]: accent })}
            >
              Save Remark
            </div>
          </div>
