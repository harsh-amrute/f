...
import { BPRViewTableGrid } from "../../../MTA/SupplyChainIntelligenceHub/BPR/styles.css";
...
const OrderElapsedGrid = ({
  isTrue,
  data,
  deptName,
  selectedOrderCount,
  highAgeingdata,
}: orderElapsedGridProps) => {
  const { user } = useUserData();
  const themeUi = user.user.theme_ui;
  const [isLeftPanelOrderStatusOpen, toggleLeftPanelOrderStatus] =
    useState<boolean>(false);
  const [isleftPanelElapsedTimeOpen, toggleLeftPanelElapsedTime] =
    useState<boolean>(false);
  const [isRightPanel, toggleRightPanel] = useState<boolean>(false);
  const [leftPanelActiveTab, SetLeftPanelActiveTab] =
    useState<string>("Order_Status");
      ...
      themeUi !== "REGALBLAZE" ? (
        isLeftPanelOrderStatusOpen ? (
          <img
            ...
            src="/assets/img/mto/DeptWiseBmReport/arrowup.svg"
          />
        ) : (
          <img
            className={IconWrapper}
            src="/assets/img/mto/DeptWiseBmReport/arrowdown.svg"
          />
        )
      ) : isLeftPanelOrderStatusOpen ? (
        <img
          className={IconWrapper}
          src="/assets/img/mto/DeptWiseBmReport/arrowup-regal.svg"
        />
      ) : (
        <img
          className={IconWrapper}
          src="/assets/img/mto/DeptWiseBmReport/arrowdown-regal.svg"
        />
      )
    ) : val == 2 ? (
      themeUi !== "REGALBLAZE" ? (
  ...
  const transformToOrderStatusRowData = () => {
    ...
    for (const [orderId, orderDetails] of Object.entries(data)) {
      ...
      const li =
        typeof orderDetailsObj.li === "string" ? orderDetailsObj.li : "";
      const tq =
        typeof orderDetailsObj.tq === "number" ? orderDetailsObj.tq : 0;
  ...
  const filterMaxElapsedTimePerDept = (rowData: RowData[]): RowData[] => {
    ...
    rowData.forEach((row) => {
      ...
      if (
        !deptMap.has(row.dept) ||
        parseElapsedTime(deptMap.get(row.dept)!.et) < currentElapsedTimeInDays
      ) {
  ...
  useEffect(() => {
    if (data) {
      ...
      const ElapsedTimeRowData = filterMaxElapsedTimePerDept(
        ElapsedTimeAllRowData
      );
  ...
  const isOrderActive = leftPanelActiveTab === "Order_Status";
  const isElapsedActive = leftPanelActiveTab === "Elapsed_Time";
  ...
  const tabBg = (isActive: boolean) => (isActive ? activeBg : "white");
  ...
  return isTrue ? (
      ...
      <div className={BPRViewTableWrapper}>
        <div className={BPRViewTablePrefixWrapper}>
          <div
            ...
            aria-selected={isOrderActive}
            style={assignInlineVars({
              [tabTextColorVar]: isOrderActive ? "#FFFFFF" : "inherit",
              [tabZIndexVar]: String(isOrderActive ? 1 : 0),
              [tabMarginLeftVar]: "0",
              [tabPaddingLeftVar]: "0",
              [tabBgVar]: tabBg(isOrderActive),
            })}
          >
            Order Status
          </div>
          <div
            ...
            aria-selected={isElapsedActive}
            style={assignInlineVars({
              [tabTextColorVar]: isElapsedActive ? "#FFFFFF" : "inherit",
              [tabZIndexVar]: String(isElapsedActive ? 1 : 0),
              [tabMarginLeftVar]: "0",
              [tabPaddingLeftVar]: "0",
              [tabBgVar]: tabBg(isElapsedActive),
            })}
          >
            Elapsed Time
          </div>
        ...
        <div
          className={BPRViewTableGrid}
          style={{ padding: "10px", borderRadius: 0 }}
        >
          {leftPanelActiveTab === "Order_Status" ? (
            <div className={ExpansionWrapper}>
              <div
                className={ExpansionHeader}
                style={{
                  borderBottom: isLeftPanelOrderStatusOpen
                    ? "solid 1px #E3ACC9"
                    : "none",
                }}
              >
                <div className={ExpansionHeaderGroup}>
                  <span className={ExpansionHeaderNormalText}>
                    Selected Orders :
                  </span>
                  <span className={ExpansionHeaderColoredText}>
                    {selectedOrderCount}
                  </span>
                </div>
                <div
                  className={ExpansionHeaderGroup}
                ...
                <div
                  className={ExpansionHeaderGroup}
                  onClick={() =>
                    toggleLeftPanelOrderStatus(!isLeftPanelOrderStatusOpen)
                  }
              ...
              {isLeftPanelOrderStatusOpen && (
                <div className={ExpansionContent}>
                  <div className={VFWrapper} style={{ height: "400px" }}>
                    <VFTable
                      {...agGridProps}
            ...
            <div className={ExpansionWrapper}>
              <div
                className={ExpansionHeader}
                style={{
                  borderBottom: isleftPanelElapsedTimeOpen
                    ? "solid 1px #E3ACC9"
                    : "none",
                }}
              >
                ...
                <div
                  className={ExpansionHeaderGroup}
                  onClick={() =>
                    toggleLeftPanelElapsedTime(!isleftPanelElapsedTimeOpen)
                  }
                  style={{ marginLeft: "auto" }}
                >
                  {dropDownButton(2)}
      ...
      <div className={BPRViewTableWrapper}>
        <div className={BPRViewTablePrefixWrapper}>
          <div
            ...
            aria-selected
            style={assignInlineVars({
              [tabTextColorVar]: "#FFFFFF",
              [tabZIndexVar]: String(1),
              [tabMarginLeftVar]: "0", // marLeft = false
              [tabPaddingLeftVar]: "0",
              [tabBgVar]: "red", // bgColor="red"
            })}
          >
            <img
              className={HigHAgeingIconWrapper}
        ...
        >
          <div className={ExpansionWrapper}>
            <div
              className={ExpansionHeader}
              style={{
                borderBottom: isRightPanel ? "solid 1px #E3ACC9" : "none",
              }}
            >
              <div className={ExpansionHeaderGroup}>
                <span className={ExpansionHeaderNormalText}>
              ...
              <div
                ...
                style={{ marginLeft: "auto" }}
              >
                <span className={ExpansionHeaderNormalText}>Min Ageing :</span>
                <span className={ExpansionHeaderColoredText}>
                  {minMaxAge.min}
                </span>
              </div>
              <div
                ...
                style={{ marginLeft: "auto" }}
              >
                <span className={ExpansionHeaderNormalText}>Max Ageing :</span>
                <span className={ExpansionHeaderColoredText}>
                  {minMaxAge.max}
                </span>
              </div>
              <div
                className={ExpansionHeaderGroup}
                onClick={() => toggleRightPanel(!isRightPanel)}
    ...
    <div className={NoDataAvailableContainer} style={{ marginBottom: "2rem" }}>
      <div className={NoDataToShowDiv}>
