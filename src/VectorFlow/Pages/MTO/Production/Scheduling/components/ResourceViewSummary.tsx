...
const ResourceViewSummary = ({ ResourceData, setExcelGridRef }: any) => {
  ...
  const allWorkStations: any = new Set(
    Object.values(ResourceData.Resource_Data || {}).map(
      (r: any) => r.work_station
    )
  );

  const [activeWorkStation, setActiveWorkStation] =
    useState<any>("All Work Stations");
  ...
  const generateSummaryData = () => {
    ...
    filteredWorkStations.forEach((ws) => {
      ...
      const totalTimeSpanHours =
        minStartDate !== Infinity && maxEndDate !== -Infinity
      ...
      Object.keys(ResourceData.Task_master || {}).forEach((type) => {
        ...
        if (active === "Percentage Wise") {
          entry[type] =
            totalTimeSpanHours > 0 ? (duration / totalTimeSpanHours) * 100 : 0;
      ...
      if (active !== "Percentage Wise") {
        if (active === "Count Wise") {
          ...
          const totalTaskDuration = Object.values(taskTypeDuration).reduce(
      ...
      <div className={gridWrapper}>
        ...
        <div className={filterSection}>
          <select
            ...
            value={activeWorkStation || ""}
            onChange={(e: React.ChangeEvent<HTMLSelectElement>) => {
              setActiveWorkStation(e.target.value);
            }}
          >
            ...
            {options.map((opt) => (
              <button
                ...
                style={assignInlineVars({
                  [toggleBgVar]: active === opt ? "#b23a7d" : "transparent",
                  [toggleColorVar]: active === opt ? "#fff" : "#555",
                  [toggleHoverBgVar]: active === opt ? "#b23a7d" : "#f0f0f0",
                })}
