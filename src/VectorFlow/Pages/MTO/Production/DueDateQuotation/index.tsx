...
import {
  ...
  useGetCCRGroupMaster,
  useGetCCRItemTypeMappingMaster,
  useGetCCRMasterData,
  useGetDailyWorkingCalendar,
  useGetFOLData,
  useGetLineCCRDetails,
  useGetMarketOperatingLeadTimeMasterData,
  useGetUIConfig,
  useGetFilteredOrdersForDDQ,
  useGetOrdersForExcelDDQ,
  useUpdateScheduleOrders,
...
const DueDateQuotation = () => {
  ...
  const [rowsSelectedForAssignment, setRowsSelectedForAssignment] =
    useState<any>(false);
  ...
  const { mutateAsync: getCCRItemTypeMappingMaster } =
    useGetCCRItemTypeMappingMaster();
  ...
  const { mutateAsync: getMarketOperatingLeadTimeMasterData } =
    useGetMarketOperatingLeadTimeMasterData();
  ...
  const { data: UIConfig, isLoading: isUIConfigLoading } =
    useGetUIConfig("DueDateQuotation");
  const { mutateAsync: getUIConfigData } = useGetUIConfigData();
  const {
    mutateAsync: getFilteredOrdersForDDQ,
    isLoading: isFilteredDataLoaded,
  } = useGetFilteredOrdersForDDQ();
  const {
  ...
  const {
    mutateAsync: updateScheduleOrders,
    isLoading: isUpdateScheduleOrders,
  } = useUpdateScheduleOrders();
  ...
  const { mutateAsync: getPageWiseFilterData /*isLoading*/ } =
    useGetFilterData();
  const { mutateAsync: getFilteredOrdersForExcelDDQ } =
    useGetOrdersForExcelDDQ();
  ...
  const columnDefs = useMemo(() => {
    return getColumnDefinations(
      UIConfig?.data ? UIConfig?.data?.data : [],
      customization,
      extras
    );
  }, [isUIConfigLoading]);
    ...
    const fetchDBRSettings = async () => {
      try {
        ...
        if (DBRSettings && DBRSettings.length) {
          const isDDQFromUI =
            DBRSettings.find((data: any) => data.flag === "IsDDQFromUI")
              ?.value == "0"
              ? false
              : true;
          if (!isDDQFromUI) {
    ...
    try {
      ...
      if (allBufferMaster) {
        allBufferMaster.forEach((master: any) => {
          if (
            "production".includes(master.buffer_type.buffer_type.toLowerCase())
          ) {
            prodMaster.push({
              label: master.buffer_code,
              value: master.buffer_id,
              size: master.buffer_size,
            });
          } else if (
            "procurement".includes(master.buffer_type.buffer_type.toLowerCase())
          ) {
            procMaster.push({
              label: master.buffer_code,
              value: master.buffer_id,
      ...
      ccrGroupData.forEach((group: any) => {
        const obj: any = {
          label: group.ccr_group_code,
          value: group.ccr_group_id,
          ccrs: [],
        };
        // let minFOL = Infinity
        let minFol = Infinity;
      ...
      const MarketLeadTimeMasterData =
        await getMarketOperatingLeadTimeMasterData();
      ...
      setMasters({
        ...
        prodMaster,
        ccrGroups,
        CCRItemTypeMappingMaster,
        FOL,
        CCRMaster,
        WorkingCalender,
        MarketLeadTimeMaster,
        DBRSettings,
      });
    ...
    switch (step) {
      ...
      case 3: {
        return (
          ...
          >
            <img src="/assets/img/mto/dueDateQuotation/calender.svg" /> Schedule
          </div>
        );
      }
      default: {
  ...
  const getUpdatedFilterData = async (
    ...
    if (isExcelExport) {
      ...
      try {
        const response = await getFilteredOrdersForExcelDDQ({
          body,
          isExcelExport: 1,
          report_name: FilterPageName.Prod_DDQ,
          unSch: unScheduled,
      ...
      try {
        const formatedFilters = formatFilterJSON(appliedFilters);
        const data: any = await getFilteredOrdersForDDQ({
          page: currentPage,
          unSch: unScheduled,
  ...
  return (
    <div
      ...
      style={{
        height: step === 2 && rowsSelectedForAssignment ? "130vh" : "100%",
      }}
    >
      {step === 1 ? (
        <MTOActionToolBar
          ...
          quickFilter={
            step === 1 ? (
              ...
              >
                <Checkbox
                  checked={unScheduled}
                  onChange={(e: any) => setUnScheduled(e.target.checked)}
                  theme={themeUi}
                />
      ...
      {(isFilteredDataLoaded ||
        ...
        isUpdateUserConfig ||
        isGetUserConfig ||
        isUpdateScheduleOrders) && <OverlayLoader />}
      {getCurrentStep()}
      <VFModalCard
        key={"key2"}
        openModal={showModal}
      ...
      >
        <div style={{ margin: "0 2rem" }}>
          ...
          >
            Any unsaved changes will be discarded, <br /> Are you sure you want
            to go back ?
          </div>
          <div
            style={{
      ...
      <div
        className={Footer}
        style={
          rowsSelectedForAssignment && step == 2
            ? {
                position: "fixed",
                bottom: 0,
                background: "white",
        ...
        disabled={disabled || isScheduleDisabled }
          onClick={() => {
            if (step == 1) {
              ...
              const isDDQActiveFlag = masters.DBRSettings.find(
                (setting: any) =>
                  setting.flag == "IsDDQActive" && setting.value == 1
              );

              // const isDDQActiveFlag = false;
              assignmentRef.current.onConfirm().then((data: any) => {
                if (data && isDDQActiveFlag) {
                  //TODO: what to do when the flag is false and no changes are made, what message to show
                  setStep(step + 1);
                }
              });
