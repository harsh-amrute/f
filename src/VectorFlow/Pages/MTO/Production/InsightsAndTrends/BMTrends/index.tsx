...
const BMTrends = () => {
  ...
  const getBMTrendData = async () => {
    const data = await getBMTrendsData({
      plant: selectedPlant?.value || undefined,
    });
  ...
  return (
    <div className={BMTrendWrapper}>
      ...
      />
      <div
        className={BMTrendsChartWrapper}
        style={{ maxHeight: "95%", paddingLeft: "20px", paddingBottom: "10px" }}
      >
        <SplitGraphContainer
          ...
          tableTitle={`Overall BM Trend (${moment(date)
            .subtract(horizonDays - 1, "days")
