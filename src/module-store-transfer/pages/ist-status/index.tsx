...
import { AlertNoRecords, SCIstStatusRow, SCIstStatusTable } from "./styles.css";
...
const istStatus = () => {
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      setCurrentAction("edit");
      if (pivotTableState.rows || currentAction === "deleting") {
        await getListView(originalDataComponent);
      }
    }
    init();
  }, [currentAction]);
  ...
  useEffect(() => {
    async function init() {
      ...
      const dataComponentList = JSON.parse(
        JSON.stringify(await Promise.all(promises))
      );
  ...
  return (
    <div
      className={`${SCIstStatusRow} ${
        currentAction === "edit" ? "edit-state" : ""
      }`}
    >
      {dataComponent.length === 0 && !loadingView && (
        <div className={AlertNoRecords}>{t("ISTStatus.notify.noRecords")}</div>
      )}
      {dataComponent.length !== 0 && !loadingView && (
        <>
          <ViewComponent
          ...
          />
          <div className={SCIstStatusTable}>{/* pivot (commented) */}</div>
        </>
      )}
    </div>
  );
