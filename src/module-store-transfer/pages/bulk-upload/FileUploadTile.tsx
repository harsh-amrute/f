        ...
        <div className={leftCommonComUploadWrapper}>
          ...
          >
            ...
            <div className={headerSection} style={{cursor: 'pointer'}} >
              <div className={headerText}>{"Upload File"}<p style={{fontSize: '0.9rem', fontWeight: 'normal'}}>{" (upto 100 users only)"}</p></div>
              {file?.name &&
              <div className={subText}>{`Selected file :  ${file.name}`}</div>

              }
              <div className={subText} style={{fontWeight: 'bold'}}>{(!file?.name)?"Click here to choose a file to upload":"Click here to change file"}</div>
            </div>
          </div>
