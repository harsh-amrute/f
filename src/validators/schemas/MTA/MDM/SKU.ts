...
export const SKUSchema = Joi.object({
    sc:Joi.string().empty().invalid(null).max(MAX_CODE_LENGTH).custom(commonValidatorWithSeperator).required().messages(generateCommonMessages('SKUCode')),
    sd:Joi.string().empty().max(MAX_NAME_LENGTH).custom(DescriptorValidator).required().messages(generateCommonMessages("Description")),
    ec:Joi.number().integer().empty().greater(MIN_DECIMAL_VAL).max(MAX_INT_VAL).messages({'number.unsafe':`Elephant Order Capping should be less than ${MAX_DECIMAL_VAL}`}),
