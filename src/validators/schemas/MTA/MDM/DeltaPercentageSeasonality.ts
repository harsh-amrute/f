...
export const DeltaPercentageSeasonalitySchema = Joi.object({
    ...
    sd:Joi.string(),
    ed:Joi.string(),
    dnp:Joi.number().integer().min(0).max(100).messages({'number.unsafe':`DeltaNormPercentage should be <= 100`}),
    ulc:Joi.number().integer().messages({'number.unsafe':`UpperLowerCap should be less than ${MAX_INT_VAL}`}),
    bd:Joi.number().messages({'number.unsafe':`BuildupDuration should be less than ${MAX_INT_VAL}`}),
