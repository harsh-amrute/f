import Joi from "joi";
import { commonValidator, generateCommonMessages,defaultJOIOptions,MAX_CODE_LENGTH,MAX_CUSTOM_ATTRIBUTE_LENGTH,MIN_DECIMAL_VAL,MAX_NAME_LENGTH,MAX_DECIMAL_VAL,CommonSchema} from "../../../commons";
...
export const SKULocationSchema = Joi.object({
    ...
    ParentWhCode:Joi.string().empty().max(MAX_CODE_LENGTH).custom(ParentWhCodeValidator).required().messages({...generateCommonMessages('ParentWhCode')}),
    ParentWhName:Joi.string(),
    Norm:Joi.number().integer(),
    MinNorm:Joi.number().integer(),
    ...
    MinOrderCount:Joi.number().integer().min(1),
    PackSize:Joi.number().integer().min(1),
    SpikeThreshold:Joi.number().min(MIN_DECIMAL_VAL).max(MAX_DECIMAL_VAL),
