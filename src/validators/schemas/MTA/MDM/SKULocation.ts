...
export const SKULocationSchema = Joi.object({
    ...
    wc:Joi.string().empty().max(MAX_CODE_LENGTH).custom(commonValidatorWithSeperator).required().messages(generateCommonMessages('wc')),
    wd:Joi.string().empty().max(MAX_NAME_LENGTH),
    pwc:Joi.string().empty().max(MAX_CODE_LENGTH).custom(commonValidatorWithSeperator).required().messages(generateCommonMessages('pwc')),
    pd:Joi.string(),
    n:Joi.number().integer(),
    mn:Joi.number().integer().custom(MNValidator).messages(SKULocationMessages('mn')),
    rlt:Joi.number().integer().max(MAX_DECIMAL_VAL).custom(RLTValidator).allow(null).messages(SKULocationMessages('RLT')),
    rcp:Joi.number().integer().max(MAX_DECIMAL_VAL).custom(RCPValidator).allow(null).messages(SKULocationMessages('RCP')),
    gcp:Joi.number().integer().max(MAX_DECIMAL_VAL).custom(GCPValidator).allow(null).messages(SKULocationMessages('GCP')),
