...
describe('VFRangeSlider', () => {
  it('renders with default props', () => {
    const { getByRole } = render(<VFRangeSlider min={0} max={100} strictMode={false} width={300} defaultValue={50} handleChange={jest.fn()}/>);
  ...
  it('updates value on slider change', () => {
    const { getByRole } = render(<VFRangeSlider min={0} max={100} strictMode={true} width={300} defaultValue={50}milestones={[0,30,40,60,70,100]} handleChange={jest.fn()} />);
    const inputRange = getByRole('slider');
