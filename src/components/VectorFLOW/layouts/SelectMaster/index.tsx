...
const SelectMaster = ({
  ...
  useEffect(() => {
    ...
    if (masterIdsArray.length > 0) {
      masterIdsArray.map((item: any) =>
        setFilterButtonStatus([...filterButtonStatus, item])
      );
    }
  }, []);
    ...
    if (currentUrl.includes("/view-modify")) {
      ...
      if (match) {
        ...
        if (!currentUrl.includes("?")) {
          ...
          newUrl =
            baseUrl + window.location.search + "&" + paramName + "=" + masterId;
        }
      }
  ...
  const onClickFilterButton = (currMaster: MDMMasterState) => {
    ...
    if (
      getFilterButtonStatus(currMaster.id) ||
      masters.find(
        (selectedMaster: MDMMasterState) => selectedMaster.id === currMaster.id
      )
    ) {
      removeFromSelectedMaster(currMaster.id);
      setFilterButtonStatus(
        filterButtonStatus.filter(
          (masterId: number) => masterId !== currMaster.id
        )
      );
  ...
  return (
    <div className={Container} style={{ zoom: "var(--default-zoom)" }}>
